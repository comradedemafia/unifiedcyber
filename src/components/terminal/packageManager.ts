...
import { saveFileToSupabase } from "./kaliFileSystem";
...
function aptInstall(pkgs: string[], assumeYes: boolean): CmdResult {
  ...
  for (const pkg of toInstall) {
    ...
    // persist installed package state (best-effort)
    (async () => {
      try {
        await supabase.from("installed_packages").upsert({ name: pkg, version: packageDB[pkg]?.version || null, installed: true, updated_at: new Date().toISOString() });
      } catch (e) {
        console.debug("persist installed package failed", e);
      }
    })();
    // create package config files if the package defines them
    if (packageConfigs[pkg]) {
      for (const cfgPath of packageConfigs[pkg]) {
        const content = `# Default ${pkg} configuration\n# Generated by package manager simulation\n`;
        (async () => {
          try {
            await saveFileToSupabase(cfgPath, { type: "file", content, size: content.length, permissions: "-rw-r--r--", owner: "root", modified: new Date().toISOString() } as any);
          } catch (e) {
            console.debug("persist package config failed", e);
          }
        })();
      }
    }
...
function aptRemove(pkgs: string[], purge: boolean): CmdResult {
  ...
  for (const pkg of toRemove) {
    ...
    if (purge) {
      ...
      if (configs) {
        for (const c of configs) {
          lines.push(`  Removing ${c} ...`);
          (async () => {
            try {
              await supabase.from("file_store").delete().eq("path", c);
            } catch (e) {
              console.debug("persist purge delete failed", e);
            }
          })();
        }
      }
    ...
    (async () => {
      try {
        await supabase.from("installed_packages").upsert({ name: pkg, version: packageDB[pkg]?.version || null, installed: false, updated_at: new Date().toISOString() });
      } catch (e) {
        console.debug("persist remove package failed", e);
      }
    })();
  }
